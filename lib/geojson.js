...
export function communeContour(commune) {
...
export function contoursToGeoJson(items, converToFeature) {
  const itemsWithCtr = items.filter(item => item.contour)

  return {
    type: 'FeatureCollection',
    features: itemsWithCtr.map(converToFeature)
  }
}
